...
async fn create_stream(to_addr: SocketAddr) -> Result<TcpStream, io::Error> {
    match TcpStream::connect(to_addr).await {
        Err(e) => {
            eprintln!("ERROR: Couldn't connect to {to_addr}");
            Err(e)
        },
        Ok(s) => Ok(s)
    }
...
// DEAD_CODE: Could use this if we create socket pool to fetch 
#[allow(dead_code)]
async fn create_stream_from_port(from_port: u16, to_addr: SocketAddr) -> Result<TcpStream, io::Error> {
    ...
    match socket.connect(to_addr).await {
        Ok(stream) => Ok(stream),
        Err(error) => {
...
impl<'a> TfFile<'a> {
    ...
    async fn make_request(&mut self, from_port: u16, to_addr: SocketAddr) -> Result<(), io::Error> {
        ...
        let mut stream = create_stream(to_addr).await?;
...
pub unsafe extern "C" fn request_blocking(files: *const TfFileNative, count: c_ulong, to_addr: *const c_char, list_dirs: c_int) -> *const c_char {
    ...
    if list_dirs == 0 {
        ...
        if !any_success_conv {
            set_output_string("ERROR: couldn't convert any dir names to utf8").expect("Error msg must convert to a valid c string");
            return NULL;
        }
    }
